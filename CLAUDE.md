...
- **BatchRenderer**: Optional Madrona-based batch renderer (`gs_madrona`) for GPU-only headless rendering. Uses 6 cardinal directional lights (`castshadow=False`, intensity=1.5) for direction-independent illumination. Requires `gs_madrona` built from source with uint8 clamp fix (see `Dockerfile.runpod`). Can be forced in single-env mode via `use_batch_renderer=True` constructor param (requires `gs.init(backend=gs.cuda)` before env creation).
